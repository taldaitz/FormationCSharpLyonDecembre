...
namespace ADO.NET
{
    class Program
    {
        ...
        static void ConnectedSelect()
        {
            ...
            SqlConnection connexion = GetConnection();
            ...
            try
            {
                ...
                while (reader.Read())
                {
                    ...
                    Console.WriteLine(reader["ContactTitle"]);
                }
                reader.Close();
            }
            catch (Exception ex)
        ...
        static void ConnectedUpdate()
        {
            ...
            // Requête SQL
            SqlCommand updateCommand = new SqlCommand("UPDATE Customers SET ContactName = 'Thomas Aldaitz' WHERE CustomerID = 'ALFKI'");
        ...
        string nom, prenom, login, password, description, fonction;

        public void PrintSingleField(string fieldName, string fieldValue)
        {
            if (!String.IsNullOrEmpty(fieldValue))
                Console.WriteLine("{1} : {0}", fieldName, fieldValue.Trim());
        }
        public void PrintFields ()
        {
            PrintSingleField("Nom", nom);
            PrintSingleField("Prenom", prenom);
            PrintSingleField("Login", login);
            //...
        }
        ...
        {
            SqlConnection connexion = new SqlConnection();
            connexion.ConnectionString = @"Data Source=(localdb)\MSSQLLocalDB;Initial Catalog=Northwind;Integrated Security=True;Connect Timeout=30;Encrypt=False;TrustServerCertificate=False;ApplicationIntent=ReadWrite;MultiSubnetFailover=False";
            return connexion;
        }
